...
# Coindex1igurações do banco de dados
DATABASE = 'portaria.db'

app.secret_key = 'sua_chave_secreta'


def criar_tabela_userconsulta():
    try:
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS userconsulta (
...
def verificar_credenciais(username, password):
    try:
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM userconsulta WHERE username = ? AND password = ?", (username, password))
...
def index1():
    ...
    if request.method == 'POST':
        ...
        return render_template('index1.html', registros2=registros2)
    else:
...
@app.route('/editar2/<int:registros2_id>', methods=['GET', 'POST'])
def editar2(registros2_id):
    if 'username' not in session:
        return redirect(url_for('login1'))
    ...
    if request.method == 'POST':
        ...
        if atualizar_registros2(registros2_id, data, horario, nome_motorista, cpf, placa, nota_fiscal, itens, fornecedor, destino,
            ...
            return 'Ocorreu um erro ao atualizar o registros2.'
    else:
        registros2 = obter_registros2()[registros2_id - 1]
        return render_template('editar2.html', registro=registros2)
...
if __name__ == '__main__':
    criar_tabela_userconsulta()
    criar_tabela_registros2()

    app.run(debug=True, host='192.168.1.67', port=5004)